...
```{r}
education_orig = read.csv(file = "../../data/education.csv")
```
...
> `num` is the number of observations that percentage of female bachelors greater than percantage of male bachelors after dropping NA values

> `n` is total number of observations available after dropping NA values

> `p_1` is the proportion of which female bachelor percentage is larger than male bachelor percentage after dropping NA values 

> `p_2` is the proportion of which female bachelor percentage equals male bachelor percentage after dropping NA values
...
```{r}
ggplot(education_orig,aes(x=pct_f_bach,y=pct_m_bach))+geom_point()+geom_smooth(color="blue",se=FALSE)+geom_abline(intercept=0, color="red",slope=1)+xlab("percentage of female bachelors")+ylab("percentage of male bachelors")+ggtitle("Female v.s. Male Bachelor Percentage Among Zip Codes in Wisconsin")
```
...
> Male education level tends to be lower than female.

  2c. What do you think might be true about the zip codes with strong outliers, for example, where percentages are 0% or 100%?

> It may be an error when data collecting OR population of those regions are too small.
...
education = education_orig %>% 
  ...
  drop_na(zip,pct_m_bach,pct_f_bach) %>% 
  rename(male=pct_m_bach,female=pct_f_bach) %>% 
  pivot_longer(-c(zip),names_to = "sex",values_to = "pct_bachelors") %>% 
  arrange(zip,sex)
```
...
```{r}
education %>% print(n=10,width=Inf)
```
...
```{r}
obesity = read.csv(file = "../../data/obesity.csv")
obesity = obesity %>% 
  mutate(zip=as.character(zip)) %>% 
...
ggplot(adult_pop_sum,aes(x=total_pop,y=sex))+geom_boxplot()+xlab("Total Population")+ylab("Sex")+ggtitle("Total Population Distribution by Gender")
```
...
```{r}
Q5 = left_join(adult_pop_sum,education) 
  Q5 %>% 
  filter(pct_bachelors == 0.0 | pct_bachelors == 100.0) %>% 
...
> zip 53706, that is UW-Madison, has 100% bachelors for women.
...
```{r}
Q5 %>% 
  mutate(estimated_bachelors=total_pop*(pct_bachelors/100)) %>% 
  drop_na(pct_bachelors,total_pop) %>% 
  group_by(sex) %>% 
  summarize(pct_m=sum(estimated_bachelors)*100/sum(total_pop))
```

> The percentages of women and men ages 25 and older in Wisconsin with a bachelors degree are 29.8 and 27.5 repectively.
...
adult_obesity_sum = obesity %>% 
  ...
  filter(remain_row==8) %>% 
  group_by(zip,sex) %>% 
  summarize(obese_pop=sum(obese),total_pop=sum(pop))
adult_obesity_sum %>% print(n=10)
```
...
```{r}
temp = inner_join(adult_obesity_sum,education,by=c('zip','sex')) %>% 
  mutate(pct_obese=obese_pop/total_pop)
ggplot(temp, aes(x=pct_bachelors,y=pct_obese))+geom_point(aes(col = sex))+geom_smooth(aes(col=sex),se=FALSE)+xlab("education percentage")+ylab("obese percentage")+ggtitle("Obesity Percentage vs Education Percentage in Wisconsin")
```

> Female tends to be more obese than male at low educated range. But both male and female obese percentage tends to decrease as education percentage increases.