...
Data in *lake-monona-winters-2022.csv* is similar to that in the Lake Mendota data set from lecture, but contains freeze durations in Lake Monona, **and not from Lake Mendota**.
...
-   Open the `hw02.Rmd` document and answer the questions.
    You will need to complete chunks to solve the problems.
    Some problems require written responses in addition to code.

### Submission

Once you have completed all of the questions, knit the R Markdown document to create an HTML file.
To submit this assignment, go to our Canvas site and select "Assignments" on the left panel, and upload both the edited .Rmd and HTML files.
...
```{r problem2}
ggplot(monona, aes(x = duration)) +
  geom_histogram(boundary=70, binwidth=5, fill="cyan",color="forestgreen")+xlab("Days Closed with Ice") +ylab("Total") +ggtitle("Lake Monona Freeze Durations, 1855-2022")
```
...
It discards the ribbon which is used to represent uncertainty in the true value of the estimated trend line.

-   Describe the pattern of the curve.

### Response

There was a steady decline from 1850 to 2022. The rate of decrease is relatively steep from 1850 to about 1900, then it becomes gentle from 1900 to about 1960, then it becomes steep again from about 1960 to 2022. Overall, the duration has decreased from over 120 days in 1850 to over 80 days in 2022.
...
```{r problem4}
ggplot(monona, aes(y=duration)) +geom_boxplot(fill="yellow",color="magenta")+ylab("duration")+ggtitle("Lake Monona Freeze Durations, 1855-2022") 
```
...
```{r}
ggplot(monona, aes(x=ff_cat))+geom_bar()+xlab("Date of the First Freeze")+ylab("Number of Winters")+ggtitle("Lake Monona First Freeze Dates",subtitle="1855-2022")
```

6.  Briefly explain why you needed to use the command `geom_bar()` and not `geom_col()` to make the plot in the previous problem.

Since we haven't manually tallied all counts for each category before applying the command, we can only use the command 'geom_bar()" so that it'll count automatically before drawing the bar graph.

7.  The following chunk creates a scatter plot with `ff_x` on the x axis and `duration` on the y axis, with points colored by `period50`. The variable `ff_x` is a numerical coding of the first freeze date, counting days after June 30. For context, December 27 is 180 days after June 30. The default color scheme is changed to `viridis` which is friendlier to most people with various forms of color blindness. The command `geom_smooth(method = "lm", se = FALSE)` adds a straight line instead of a curve to the plot (that's the `method = "lm"` argument) and because we specified `period50` as a grouping variable by mapping it to the color aesthetic, separate lines are added for each group.

Add code to add a plot title and to provide informative axis labels.
Following examples from lecture notes, change the title of the color legend to say "Time Period" instead of "period50".
...
ggplot(monona, aes(x = ff_x, y = duration, color = period50)) +
  ...
  ggtitle("Lake Monona Freeze Durations over First Freeze Dates", subtitle="1855-2022")+
  xlab("First Freeze Date")+ylab("Duration")+guides(color = guide_legend(title = "Time Period"))
```

After making the graph, suppose that the date of the first freeze in some year was December 27, which is 180 days after June 30.
Based on an examination of the graph, briefly explain how your prediction of the total duration that Lake Monona is closed by ice would differ if the winter was in the 1870s versus the present?

Total Duration that Lake Monona is closed by ice would be longer in 1870s than in the present. Because when you find 180 on x-axis, the corresponding value on y axis for purple line (which represents 1870s' total freeze duration) is greater than the corresponding value on y axis for yellow line (which represents present's total freeze duration). This indicates that total freeze duration in 1870 is predicted to be longer than in the present.
...
ggplot(monona, aes(x = ff_x, y=duration)) +
  ...
  ylab("Duration") +
  ggtitle("Lake Monona Freeze Durations over First Freeze Dates",
          subtitle = "1855-2022") +
...
Coloring really does make it easier since it can combine all 50-year time periods into a single graph, which makes comparing relationship across all periods super easy and straightforward. 
Faceting is not that easy since it places period in separate sections which make detailed comparison of relationship across all periods hard to do, at least. However, general comparison (i.e. all four 50-year time periods have a negative relationship) is still easy using faceting.
